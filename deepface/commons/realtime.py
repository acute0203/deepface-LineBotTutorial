	...
	while(True):
		...
		if freeze == True:
			...
			if (toc - tic) < time_threshold:
				...
				if last_update_time == None or now >= (last_update_time + timedelta(minutes = 5)):
					file_name = "tmp/"+str(uuid.uuid1())+".jpg"
					cv2.imwrite(file_name, freeze_img)
					#send image to line
					sendImgToLine(file_name)
...
def sendImgToLine(fileName):
	files={'file': open(fileName, 'rb')}
	r=requests.post("http://127.0.0.1:5000/upload_file",files=files)
